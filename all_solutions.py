...
def print_solutions(date):
    solutions = get_solutions(date)
    with open(f'{date[0]:02}{date[1]:02}{date[2]}.txt', 'w') as f:
        f.write('\n'.join(format_solution(date, solution) for solution in solutions))
    print(*date, sep='\t')


def format_solution(date, solution):
    output = [['F'] * WIDTH for _ in range(HEIGHT)]
    date_rules = get_date_rules(*date)
    for d in date_rules:
        row, col = divmod(d, WIDTH)
        output[row][col] = 'H'
    for p in solution:
        if not p.startswith('Position'):
            continue
        position, *rows = p.split('\n')
        _, *coordinates = position.split(' ')
        y, x = map(int, coordinates)
        for dr in range(len(rows)):
            for dc in range(len(rows[dr])):
                cell = rows[dr][dc]
                if cell != ' ':
                    output[y+dr][x+dc] = cell
...
if __name__ == "__main__":
    ...
    with Pool() as p:
        p.map(print_solutions, product(range(1, 13), range(1, 32), range(7)))
